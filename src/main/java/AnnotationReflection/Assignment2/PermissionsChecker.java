...
public class PermissionsChecker {
	...
	static List<OperationType> findUserAllowedOperations(Annotations.Permissions[] allPermissions, User user) {
		...
		for (Annotations.Permissions permissions : allPermissions) {
			if (user.getRole().equals(permissions.role())) {

				operationTypeList.addAll(Arrays.stream(permissions.allowed()).collect(Collectors.toList()));
			}
		}
